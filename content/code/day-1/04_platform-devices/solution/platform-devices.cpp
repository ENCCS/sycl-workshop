...
main()
{
  ...
  for (const auto& p : platform::get_platforms() ) {
    ...
    for (const auto& d : p.get_devices() ) {
      ...
      std::cout << "   Global memory size " << d.get_info<info::device::global_mem_size>() / (1024.0 * 1024.0 * 1024.0) << " GiB" << std::endl;
      std::cout << "   Local memory type " << local_memory_type(d) << std::endl;
      std::cout << "   Local memory size " << d.get_info<info::device::local_mem_size>() / (1024.0) << " KiB" << std::endl;
      std::cout << "   Maximum work items dimesions " << d.get_info<info::device::max_work_item_dimensions>() << std::endl;
      std::cout << "   Maximum work group size " << d.get_info<info::device::max_work_group_size>() << std::endl;
    }
    std::cout << std::endl;
  }
